...
const Home: NextPage = () => {
  ...
  return (
    <div className={"flex justify-center items-center min-h-screen "+ getRandom(bgColors)}>
      <div className="shadow-md font-ui py-10 px-12 rounded-xl shadow-lg space-y-12 bg-white max-w-[500px]">
        <div>
  ...
  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.4} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
              ...
              <input placeholder="เลขห้อง" className="border border-gray-300 rounded-xl pl-12 pt-1.5 pb-0.5 w-full text-gray-500 text-xl"/>
            </div>
            <div className="flex items-center">
              <h1 className="text-gray-600 font-medium text-lg mr-2">ความชัด:</h1>
              <div onClick={() => {setQualityPanel((prev) => (!prev))}} className="flex relative items-center cursor-pointer">
                <PencilIcon className="w-4 h-4 text-blue-400 mb-1"/>
                <span className="text-blue-400">ปกติ</span>
                {qualityPanel && <div className="absolute top-7 bg-white w-[56px] rounded-lg shadow-lg">
                  <h2 className="text-gray-800 cursor-pointer text-sm text-center rounded-t-lg pt-1 px-2 hover:bg-gray-100">ต่ำ</h2>
                  <h2 className="text-gray-800 cursor-pointer text-sm text-center bg-blue-50 py-0.5 px-2">ปกติ</h2>
                  <h2 className="text-gray-800 cursor-pointer text-sm text-center py-0.5 px-2 hover:bg-gray-100">สูง</h2>
                  <h2 className="text-gray-800 cursor-pointer text-sm text-center pb-1 px-2 hover:bg-gray-100">สูงมาก</h2>
                </div>}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-10">
            <div className="flex relative w-[200px] h-[44px]">
              <div className="flex border border-gray-300 w-full rounded-xl">
                ...
                <button onClick={() => {setPreset((prev) => (!prev))}} className="flex items-center justify-center w-4/12 border-l border-gray-300 cursor-pointer hover:bg-gray-100 rounded-r-xl"><ChevronDownIcon className="w-5 h-5 text-gray-700"/></button>
              </div>
              {preset && <div className="absolute top-12 bg-white w-full rounded-lg shadow-lg px-6 py-4 space-y-2">
                <div className="border-b border-gray-300 py-2">
                  <h1 className="mb-1.5">ชุดสี</h1>
                  <h2 onClick={resetDefault} className="text-gray-400 mb-1 cursor-pointer">ค่าเริ่มต้น</h2>
                </div>
                <div className="border-b border-gray-300 py-2">
